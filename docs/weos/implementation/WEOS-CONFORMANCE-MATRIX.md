...
| Implementation status | `APP-008A_IMPLEMENTED_APP-008B_IMPLEMENTED_FOR_NEW_DAILYCASE_BINDINGS` |
| Conformance | `PARTIAL_STAGED_RUNTIME_CONFORMANCE` |
| Prerequisites | APP-006 exact `CaseRevision` approval; APP-007 closed `CREATE_CASE_REVISION` mutation hardening |

APP-008 authorizes staged runtime work only. It preserves APP-006 as the
...
revision. APP-008A now implements canonical revision-targeted publication
decisions. APP-008B now binds newly scheduler-created `DailyCase` rows to the
exact APP-008A `CaseRevisionPublicationDecision` and `CaseRevision`. APP-008C
`GameSession` revision binding and revision-bound learner hydration and APP-008D
attempt provenance/legacy hardening remain unimplemented.
...
| `WEOS-CONF-002` | Case revision identity                  | Approved/publicized content targets an exact revision.                               | Remediated Stage 2 `APPROVE_CASE_REVISION` requires explicit `expectedRevisionId` and targets exact `CaseRevision`; APP-007 `CREATE_CASE_REVISION` adds lineage/hash/idempotency for supported material edits; APP-008A publication decisions target exact approved revisions; APP-008B new `DailyCase` rows store `caseRevisionId`; `GameSession` and `Attempt` still do not bind to it. | Exposure and attempts preserve exact published revision or equivalent approved snapshot identity.     | `case-review.service.spec.ts`; `case-revision.service.spec.ts`; `daily-cases.service.spec.ts`; Phase 0 read path   | `PARTIAL`         | APP-008C and APP-008D still required.    |
...
| `WEOS-CONF-004` | Revision-targeted publication           | Publication targets an approved revision.                                            | APP-008A persists canonical `CaseRevisionPublicationDecision` rows for exact approved revisions and treats `Case.editorialStatus`/`publishedAt` as compatibility projection only.                                                       | Publication decision references exact approved revision.                                              | `case-publication-governance.service.ts`; `app008a-case-publication-race.e2e-spec.ts` | `PARTIAL` | APP-008B/C/D still required for exposure/provenance completion.                             |
| `WEOS-CONF-005` | Learner exposure version identity       | Learner exposure records preserve content identity.                                  | APP-008B new `DailyCase` rows carry `caseId`, `caseRevisionId`, and `publicationDecisionId`; legacy rows remain nullable/unbackfilled; gameplay hydration still reads mutable `Case`; sessions/attempts lack revision/version/hash.                              | `DailyCase`/session/attempt or equivalent exposure record stores approved published version identity. | `daily-cases.service.spec.ts`; Phase 0 read path                  | `PARTIAL`       | APP-008C and APP-008D implementation required.      |
