...
export function buildShareCardDataFromRound(
  ...
  const totalClues = Math.max(
    1,
    roundViewModel.totalClues || result?.case?.clues.length || 6,
  );
  const attemptsUsed =
    roundViewModel.resultAttemptsUsed ??
    result?.attemptsCount ??
    roundViewModel.attemptsCount;
  const cluesUsed =
    roundViewModel.resultCluesUsed ?? Math.min(attemptsUsed, totalClues);
  const isCorrect =
  ...
  return {
    ...
    caseLabel: buildShareCaseLabel({
      publicNumber:
        roundViewModel.casePublicNumber ??
        result?.case?.casePublicNumber ??
        null,
    ...
    streak,
    xpTotal:
      typeof roundViewModel.hud.xpTotal === "number"
        ? roundViewModel.hud.xpTotal
        : null,
    school: overrides.school ?? null,
    attemptLabels: roundViewModel.attemptHistory.map(
...
export function buildShareCardDataFromResult(
  ...
  const totalClues = Math.max(
    1,
    result.case?.clues.length ?? fallback.totalClues ?? 6,
  );
  const attemptsUsed =
    result.attemptsCount ??
    fallback.resultAttemptsUsed ??
    fallback.attemptsCount;
  const isCorrect =
  ...
  return {
    ...
    xpTotal:
      typeof fallback.hud.xpTotal === "number" ? fallback.hud.xpTotal : null,
    school: null,
    attemptLabels:
...
function buildTimeUsedText(
  result: GameResult | null | undefined,
  fallback: RoundViewModel,
) {
  return formatTimeUsed(
    getElapsedSecondsFromResult(result) ?? fallback.elapsedSeconds ?? null,
  );
}
...
function getFallbackAttemptLabels({
  ...
  const labels: ShareAttemptLabel[] = Array.from(
    { length: attemptsUsed },
    () => "wrong",
  );
