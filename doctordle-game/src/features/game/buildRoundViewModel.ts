...
function getHudStatus(mode: GameEngineMode, latestAttempt: GameAttempt | null) {
  ...
  if (mode.type === "FINAL_FEEDBACK") {
    return latestAttempt?.label === "correct"
      ? "Patient stabilized"
      : "Patient lost";
  }
...
function getSuccessfulResolutionViability(
  totalClues: number,
...
function getViabilityRemaining(input: {
  ...
  if (
    input.latestResult?.gameOverReason === "correct" ||
    input.latestResult?.label === "correct"
  ) {
    return getSuccessfulResolutionViability(
      input.totalClues,
      input.attemptsCount,
    );
  }
...
function getOutcomeTone(input: {
  ...
  return input.viabilityRemaining <= steadyThreshold
    ? "steady_save"
    : "early_save";
}
...
export function buildRoundViewModel({
  ...
  const gameplayVisibleClues = gameplayClues.filter(
    (clue) => clue.order <= clueIndex,
  );
  const totalClues =
    latestResult?.case?.clues.length ?? caseData?.clues.length ?? 0;
  ...
  const isTerminal =
    mode.type === "FINAL_FEEDBACK" || latestResult?.gameOver === true;
  ...
  const resultWasCorrect =
    mode.type === "FINAL_FEEDBACK"
      ? latestResult?.gameOverReason === "correct" ||
        latestResult?.label === "correct"
      : null;
  const resultAttemptsUsed =
    mode.type === "FINAL_FEEDBACK" ? attemptsCount : null;
  const resultCluesUsed =
    mode.type === "FINAL_FEEDBACK"
      ? resultWasCorrect
        ...
        : Math.max(
            1,
            totalClues || gameplayVisibleClues.length || attemptsCount || 1,
          )
      : null;
  const revealedClueCount = isTerminal
    ? totalClues
    : gameplayVisibleClues.length;
  ...
  const finalExplanation = truncateExplanationDisplayText(
    latestResult?.explanation ?? null,
  );
  const explanationAvailable = finalExplanation !== null;
  const resolvedUnavailableReason =
    mode.type === "BLOCKED"
      ? (error ?? unavailableReason ?? "No case available right now.")
      : null;
  const nextCaseAction =
    waitingCountdownText && nextCaseAt
      ? ({
          kind: "countdown",
          nextCaseAt,
          countdownText: waitingCountdownText,
        } as const)
      : ({ kind: "none" } as const);
  ...
  return {
    ...
    finalDiagnosis:
      mode.type === "FINAL_FEEDBACK" ? (latestAttempt?.guess ?? null) : null,
    finalExplanation,
    outcomeTone,
    reward,
    elapsedSeconds,
...
function getSubmitPromptLabel(input: {
  diagnosisSubmitMode: DiagnosisSubmitMode;
  diagnosisInputState: DiagnosisInputState;
  dictionaryAvailability: DiagnosisDictionaryAvailability;
}): string | null {
