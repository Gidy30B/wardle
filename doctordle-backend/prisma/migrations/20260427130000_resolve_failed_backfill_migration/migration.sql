-- Resolution migration: unblocks Prisma P3009 by marking the failed backfill
-- migration as resolved, then re-runs the backfill logic in a safe, idempotent
-- manner so the DiagnosisRegistry and DiagnosisAlias tables are properly
-- populated without risking another migration failure.

-- ─────────────────────────────────────────────────────────────────────────────
-- Step 0: Resolve the failed migration in the Prisma migrations table.
-- This clears the P3009 lock so subsequent `prisma migrate deploy` calls work.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Step 1: Ensure the unaccent extension is available (safe, idempotent).
-- ─────────────────────────────────────────────────────────────────────────────
CREATE EXTENSION IF NOT EXISTS unaccent;

-- ─────────────────────────────────────────────────────────────────────────────
-- Step 2: Create (or replace) the normalisation helper function.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Step 3: Backfill DiagnosisRegistry from legacy Diagnosis rows.
...
-- Patch any registry rows that share a canonical name with a Diagnosis but
-- were inserted without a legacyDiagnosisId (e.g. from a previous partial run).
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Step 4: Backfill canonical DiagnosisAlias entries.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Step 5: Backfill synonym DiagnosisAlias entries.
-- Uses a single CTE chain to avoid the illegal-reference issue that caused the
-- original migration to fail.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Count how many distinct legacy diagnoses share the same normalised term
-- (used to detect ambiguous synonyms that should not be accepted for matching).
term_owner_counts AS (
  SELECT
    normalized_term,
    COUNT(DISTINCT owner_id) AS owner_count
  FROM (
    SELECT sn.normalized_term, sn.diagnosis_id AS owner_id
    FROM synonym_normalized sn

    UNION

    SELECT dr."canonicalNormalized", dr."legacyDiagnosisId"
    FROM "DiagnosisRegistry" dr
    WHERE dr."legacyDiagnosisId" IS NOT NULL
  ) owners
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Step 6: Link Case rows to their DiagnosisRegistry entry.
-- ─────────────────────────────────────────────────────────────────────────────
UPDATE "Case" c
SET
...
WHERE dr."legacyDiagnosisId" = c."diagnosisId"
  AND c."diagnosisRegistryId" IS NULL;

-- ─────────────────────────────────────────────────────────────────────────────
-- Step 7: Link CaseRevision rows to their DiagnosisRegistry entry.
-- ─────────────────────────────────────────────────────────────────────────────
...
WHERE dr."legacyDiagnosisId" = cr."diagnosisId"
  AND cr."diagnosisRegistryId" IS NULL;

-- ─────────────────────────────────────────────────────────────────────────────
-- Cleanup: drop the helper function (it is recreated idempotently if needed).
-- ─────────────────────────────────────────────────────────────────────────────
