 ...
 *
 * Run:
 *   npx tsx prisma/seed/flagship-adult-epiglottitis-v4.seed.ts
 */
...
const seedVersion = 'flagship-adult-epiglottitis-v4';
...
async function ensureCaseRevision(params: {
  caseId: string;
  caseDate: Date;
  ...
  if (latestRevision) {
    await prisma.case.update({
      ...
      data: { currentRevisionId: latestRevision.id },
    });
  ...
  const revision = await prisma.caseRevision.create({
    data: {
      ...
      title: caseTitle,
      date: params.caseDate,
      difficulty: 'intermediate',
      history: clues[0].value,
      symptoms: [clues[1].value],
      ...
      diagnosisEditorialNote:
        'Seeded frontend-aligned flagship Adult Epiglottitis case revision with six supported clues, exact breakdown alignment, canonical differentials, controlled-airway reasoning, and diagnosis-level structured education.',
    },
    select: { id: true, revisionNumber: true },
  });
...
async function ensureValidationRun(params: {
  ...
  const existingRun = await prisma.caseValidationRun.findFirst({
    where: {
      caseId: params.caseId,
  ...
  return prisma.caseValidationRun.create({
    data: {
      caseId: params.caseId,
      revisionId: params.revisionId,
      source: CaseSource.MANUAL,
      ...
      findings: {
        supportedClueTypes: true,
        exactBreakdownAlignment: true,
        noEarlyDiagnosisLeakage: true,
        canonicalDifferentials: true,
        structuredEducation: true,
      },
      completedAt: now,
    },
    select: { id: true },
...
async function ensureCase(params: {
  ...
  if (existingCase) {
    const revision = existingCase.currentRevisionId
      ? { id: existingCase.currentRevisionId }
      : await ensureCaseRevision({
          caseId: existingCase.id,
          caseDate: existingCase.date,
          diagnosisRegistryId: params.diagnosisRegistryId,
        });
    ...
    console.log(
      existingCase.currentRevisionId
        ? existingCase.dailyCases.length
          ? 'Skipped existing scheduled Adult Epiglottitis case.'
          : 'Skipped existing complete Adult Epiglottitis case to avoid overwriting authored content.'
        : 'Recovered partially created Adult Epiglottitis case by attaching its missing revision.',
      {
        id: existingCase.id,
        publicNumber: existingCase.publicNumber,
  ...
  const assignedDate =
    await findAvailableInventoryPlaceholderDate(inventoryPlaceholderDate);
  ...
  const seededCase = await prisma.case.create({
    data: {
      ...
      diagnosisMappingConfidence: 1,
      diagnosisEditorialNote:
        'Seeded frontend-aligned flagship Adult Epiglottitis case with six supported clues, exact breakdown alignment, canonical differentials, controlled-airway reasoning, and diagnosis-level structured education.',
    },
    select: {
      id: true,
      title: true,
  ...
  const revision = await ensureCaseRevision({
    caseId: seededCase.id,
    caseDate: seededCase.date,
    diagnosisRegistryId: params.diagnosisRegistryId,
  });
