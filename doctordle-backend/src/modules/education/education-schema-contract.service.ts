...
const OBJECTIVE_FINDING_PATTERN =
  /\b(?:shows?|reveals?|demonstrates?|documents?|detects?|finding|findings|pattern|positive|negative|elevated|reduced|low|high|normal|abnormal|increased|decreased|leukocytosis|ketone|ketones|acidosis|consolidation|infiltrate|opacity|troponin|lactate|bnp|wbc|ph|bicarbonate|creatinine|score|ratio|level|levels|titre|titer|antibody|proteinuria|hematuria|casts?)\b/i;

const RESULT_RELATION_PATTERN =
  /\b(?:shows?|reveals?|demonstrates?|documents?|detects?|identifies|confirms|is consistent with|would show|may show|can show|typically shows|expected (?:finding|result)|positive|negative|elevated|reduced|low|high|normal|abnormal|increased|decreased)\b/i;

const CLINICAL_INTERPRETATION_PATTERN =
  /\b(?:supports|favors|favours|suggests|indicates|argues against|rules out|rules in|helps exclude|excludes?|helps distinguish|distinguishes|differentiate|separat(?:es|ing)|confirms|characterizes|raises suspicion|lowers suspicion|increases (?:the )?likelihood|decreases (?:the )?likelihood|decreases confidence|makes .{1,80} less likely|makes .{1,80} more likely|is consistent with|points toward|points away from|rather than|demonstrates|should not override|changes diagnostic|severity|threshold|risk stratifies|stratifies risk|at risk of|probability|likelihood)\b/i;

const MECHANISM_PATTERN =
  /\b(?:because|due to|results? from|caused by|arises because of|arises from|reflect(?:s|ing)?|produces|provokes|reproduces|generated by|driven by|secondary to|as .{1,80} leads to|when .{1,80} irritates?|irritat|extension|palpation|maneuver|manoeuvre|pressure|stretch|obstruction|inflammation|immune|deposition|ischemia|oedema|edema)\b/i;

const GENERIC_WHY_PATTERN =
  /\b(?:important|critical|useful|helpful|supports (?:the )?diagnosis|guides management|guides treatment|important for (?:early )?diagnosis|important for management|clinical correlation is advised)\b/i;
...
export class EducationSchemaContractService {
  ...
  hasInvestigationExpectedFinding(value: unknown): boolean {
    const pearl = this.readTypedPearl(value);
    const text = [pearl.content, pearl.whyItMatters]
      .filter(Boolean)
      .join(' ');
    ...
    return Boolean(
      pearl.content &&
        (OBJECTIVE_FINDING_PATTERN.test(pearl.content) ||
          (RESULT_RELATION_PATTERN.test(pearl.content) &&
            this.hasClinicalAnchor(text))),
    );
  }

  hasInvestigationInterpretation(value: unknown): boolean {
    const pearl = this.readTypedPearl(value);
    return this.hasClinicalInterpretation(pearl.whyItMatters, {
      requireReasoningSignal: true,
    });
  }

  hasExamMechanism(value: unknown): boolean {
    const pearl = this.readTypedPearl(value);
    return Boolean(
      (pearl.title || pearl.content) &&
        pearl.content &&
        pearl.content.split(/\s+/).filter(Boolean).length >= 6 &&
        MECHANISM_PATTERN.test(pearl.content),
    );
  }

  hasDiagnosticImpact(value: unknown): boolean {
    const pearl = this.readTypedPearl(value);
    return this.hasClinicalInterpretation(pearl.whyItMatters, {
      requireReasoningSignal: false,
    });
  }
  ...
  private hasClinicalInterpretation(
    value: string | null,
    input: { requireReasoningSignal: boolean },
  ): boolean {
    if (!value || this.isGenericWhyLayer(value)) {
      return false;
    }
    ...
    const hasInterpretiveLanguage = CLINICAL_INTERPRETATION_PATTERN.test(value);
    if (!hasInterpretiveLanguage) {
      return false;
    }

    return !input.requireReasoningSignal || this.hasClinicalAnchor(value);
  }

  private hasClinicalAnchor(value: string): boolean {
    return /\b(?:diagnosis|diagnostic|likelihood|probability|suspicion|mimic|differential|severity|risk|rule[- ]?out|disease|syndrome|inflammation|infection|ischemia|malignancy|nephropathy|nephrotic|nephritic|glomerulonephritis|podocyte|acidosis|ketosis|peritonitis|appendicitis|sepsis|embolism|infarction|pneumonia|ulcer|torsion|occlusion|glomerular|proteinuria|hematuria)\b/i.test(
      value,
    );
  }
  ...
  private isGenericWhyLayer(text: string): boolean {
    ...
    return (
      GENERIC_WHY_PATTERN.test(compact) ||
      /\b(?:can lead to complications|prompt treatment is necessary)\b/i.test(
        compact,
      )
    );
  }
