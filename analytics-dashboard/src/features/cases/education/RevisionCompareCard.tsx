  ...
  return (
    <section className="rounded-lg border border-[var(--color-navy-border)] bg-white/[0.03] p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {comparison ? (
          <span className="rounded-full border border-[var(--color-navy-border)] bg-white/5 px-2.5 py-1 text-xs font-semibold text-slate-300">
...
function RevisionSelect({
  ...
  return (
    <label className="block space-y-2">
      ...
      <select
        className="w-full rounded-lg border border-[var(--color-navy-border)] bg-slate-950/70 px-3 py-2 text-sm font-medium text-slate-100 outline-none transition focus:border-[var(--color-teal)]"
        value={value ?? ''}
        onChange={(event) =>
...
function ComparisonSummary({
  ...
  return (
    <EditorialEntity
      eyebrow="Checkpoint comparison"
      title={`v${comparison.fromVersion} -> v${comparison.toVersion}`}
      subtitle="Decision-relevant quality movement between saved education versions."
      tone={regressedSections.length ? 'danger' : unchanged ? 'neutral' : 'success'}
      state={
        <>
          <WorkflowStateInline
            label={unchanged ? 'No meaningful change' : 'Improved'}
            tone={unchanged ? 'neutral' : 'success'}
          />
          {regressedSections.length > 0 ? (
            <WorkflowStateInline label="Regressed" tone="danger" />
          ) : null}
        </>
      }
    >
      ...
      <StreamDisclosure
        title="Section-level changes"
        summary={`${comparison.changedSections.length} changed section${
          comparison.changedSections.length === 1 ? '' : 's'
        }`}
      >
